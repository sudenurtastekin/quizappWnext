...
export default function Sorular() {
  ...
  const question = Data.filter(item => item.kategoriId === 4);
  const currentQuestion = question[step - 1];

  const progressPercentage = (step / question.length) * 100;

  useEffect(() => {
    const chosenKonu = konular.find(k => k.id === 4);
        ...
        <div className="results-screen">
          ...
          <div className="ScoreArea">
            <div className="ResultsTopic">
                <Image src={selectedImage} alt={selectedKonu} width={50} height={50} style={{backgroundColor: '#F6E7FF'}} />
                <h4>{selectedKonu}</h4>
              </div>
            <h1>{score}</h1>
