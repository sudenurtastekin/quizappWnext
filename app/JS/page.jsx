...
export default function Sorular() {
  ...
  const question = Data.filter(item => item.kategoriId === 3);
  const currentQuestion = question[step - 1];

  const progressPercentage = (step / question.length) * 100;

  useEffect(() => {
    const chosenKonu = konular.find(k => k.id === 3);
        ...
        <div className="results-screen">
          ...
          <div className="ScoreArea">
            <div className="ResultsTopic">
                <Image src={selectedImage} alt={selectedKonu} width={50} height={50} style={{backgroundColor: '#EBF0FF'}} />
                <h4>{selectedKonu}</h4>
              </div>
            <h1>{score}</h1>
